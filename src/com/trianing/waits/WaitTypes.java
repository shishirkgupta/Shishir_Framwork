 ...
 * 
 * @author Naveen
 * @see this class shall have explicit wait for any location any type on either
 *      present, visibility etc and participants can have more of these
 *      explicitWait depending on requirement
 * @since 16-Dec-2018
 */
public class WaitTypes {
	...
	public WebElement presenceElementLocated(String locator, int timeout) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, timeout);
			WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(By.id(locator)));
			System.out.println("Element Located");
			return element;
		} catch (Exception e) {
			System.out.println("Element Not Located " + e);
